...
class M_datapenyedia extends CI_Model
{
    ...
    // get nib
    private function _get_data_query_neraca_keuangan()
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_neraca_keuangan');
        $this->db->where('tbl_vendor_neraca_keuangan.id_neraca', 1);
        $i = 0;
        ...
        if (isset($_POST['order'])) {
            ...
            $this->db->order_by('tbl_vendor_neraca_keuangan.id_neraca', 'ASC');
        }
    }

    public function gettable_neraca_keuangan() //nam[ilin data pake ini
    {
        $this->_get_data_query_neraca_keuangan(); //ambil data dari get yg di atas
        if ($_POST['length'] != -1) {
            $this->db->limit($_POST['length'], $_POST['start']);
        }
        $query = $this->db->get();
    ...
    public function count_filtered_neraca_keuangan()
    {
        $this->_get_data_query_neraca_keuangan(); //ambil data dari get yg di atas
        $query = $this->db->get();
        return $query->num_rows();
