          ...
          if (response['row_akta_perubahan']) {
            ...
            if (response == 'maaf') {
                ...
                if (response['row_akta_perubahan']['sts_token_dokumen'] == 1) {
                    ...
                    $('.token_generate').html('<div class="input-group"><span class="input-group-text"><i class="fas fa-qrcode"></i></span><textarea class="form-control form-control-sm" disabled>'+response['row_akta_perubahan']['token_dokumen']+'</textarea></div>');
                } else {
...
function DekripEnkrip(id_url, type){
    ...
    var model_enkrip_perubahan = $('#model_enkrip_perubahan');
    if (type == 'dekrip') {
      modal_dekrip_perubahan.modal('show');
      $('input').attr("readonly", false);
      $('[name="id_url"]').val(id_url);
    } else {
      model_enkrip_perubahan.modal('show');
      $('input').attr("readonly", false);
      $('[name="id_url"]').val(id_url);
            ...
            Swal.fire(response['maaf'], '', 'warning')   
                ...
                Swal.fire({
                  ...
                  willClose: () => {
                    ...
                    model_enkrip_perubahan.modal('hide');
                  }
                }).then((result) => {
                  /* Read more about handling dismissals below */
                  if (result.dismiss === Swal.DismissReason.timer) {
